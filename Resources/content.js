    ...
    if (match) {
       ...
       // postID, commentID, context can be NULL, but subreddit should never be
        if (postID && commentID && context) {
            if (isAutomatic) {
                window.location.replace(`https://openinprovenance.com?subreddit=${subreddit}&postID=${postID}&commentID=${commentID}&context=${context}`);
            } else {
                window.location.replace(`provenance://reddit.com/r/${subreddit}/comments/${postID}/_/${commentID}/?context=${context}`);
            }
        } else if (postID && commentID) {
            if (isAutomatic) {
                window.location.replace(`https://openinprovenance.com?subreddit=${subreddit}&postID=${postID}&commentID=${commentID}`);
            } else {
                window.location.replace(`provenance://reddit.com/r/${subreddit}/comments/${postID}/_/${commentID}`);
            }
        } else if (postID) {
            if (isAutomatic) {
                window.location.replace(`https://openinprovenance.com?subreddit=${subreddit}&postID=${postID}`);
            } else {
                window.location.replace(`provenance://reddit.com/r/${subreddit}/comments/${postID}`);
            }
        } else {
            if (isAutomatic) {
                window.location.replace(`https://openinprovenance.com?subreddit=${subreddit}`);
            } else {
                window.location.replace(`provenance://reddit.com/r/${subreddit}`);
            }
        }
    }
}
